  ...
  class API < Grape::API
    ...
    resource :device do
      ...
      post "/upload_score" do
        device = ClientTerminal.find_by(token: params[:device_token])
        game = PublishedGame.find_or_create_by!(id: params[:game_id])

        ScoreBoard::Score.create!(client_terminal: device, published_game: game, value: params[:score])
        body false
      end
      ...
      post "/scores" do
        ...
        ScoreBoard::Score.ranked_device_game_score(device, game, limit: params[:limit]).map do |score|
          { score: score.value, rank: score.rank, percentage: (1 - (score.rank.to_f / total_score_size)) * 100 }
        end
      end
    end
