...
class CustomizeGameServiceTest < ActiveSupport::TestCase
  ...
  test 'should capture error when game modification document is invalid' do
    ...
    assert response[:reason].present?
  end
        ...
        unless File.exist?(mock_file_path)
          if STDIN.tty?
            ...
            if user_input == 'y'
              ...
              raise "Test aborted: Missing mock file #{mock_file_path} and user declined to generate one."
            end
          else
            raise "Mock file not found: #{mock_file_path}. Run tests interactively to generate mocks."
          end
        end
